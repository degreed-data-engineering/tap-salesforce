...
def resume_syncing_bulk_query(sf, catalog_entry, job_id, state, counter):
    ...
    if not bulk.job_exists(job_id):
        logger.warning("Found stored Job ID that no longer exists, resetting bookmark and removing JobID from state.")
        return counter
    ...
    for batch_id in batch_ids[:]:
        ...
        logger.warning("Finished syncing batch %s. Removing batch from state.", batch_id)
        logger.warning("Batches to go: %d", len(batch_ids))
...
def sync_records(sf, catalog_entry, state, counter, state_msg_threshold):
    ...
    logger.warning('Syncing Salesforce data for stream %s', stream)
