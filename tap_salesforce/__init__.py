...
def do_discover(sf):
    ...
    for sobject_name in objects_to_discover:
        ...
        if missing_unsupported_field_names:
            logger.warning("Ignoring the following unsupported fields for object %s as they are missing from the field list: %s",
                        sobject_name,
                        ', '.join(sorted(missing_unsupported_field_names)))

        if filtered_unsupported_fields:
            logger.warning("Not syncing the following unsupported fields for object %s: %s",
                        sobject_name,
    ...
    if unsupported_tag_objects:
        ...
        logger.warning(unsupported_tag_objects)
...
def pop_deselected_schema(
    ...
    for property_name, val in list(schema.get("properties", {}).items()):
        ...
        logger.warning(stream_name + '.' + property_name + ' - ' + str(selected))
        if not selected:
...
async def sync_catalog_entry(sf, catalog_entry, state):
    ...
    if not stream_is_selected(mdata):
        logger.warning("%s: Skipping - not selected", stream_name)
        return

    logger.warning("%s: Starting", stream_name)
    ...
    if job_id:
        with metrics.record_counter(stream) as counter:
            logger.warning("Found JobID from previous Bulk Query. Resuming sync for job: %s", job_id)
            ...
            logger.warning("Completed sync for %s", stream_name)
